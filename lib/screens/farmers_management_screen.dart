...
class _FarmersManagementScreenState
    extends State<FarmersManagementScreen> {
  ...
  Future<void> _loadFarmers() async {
    ...
    try {
      ...
      // Show only farmers, not admin accounts.
      final farmerList = users.where((user) {
        return user['role']?.toString().toLowerCase() == 'farmer';
      }).toList();
  ...
  void _searchFarmers(String query) {
    ...
    setState(() {
      filteredFarmers = farmers.where((farmer) {
        final name =
            farmer['name']?.toString().toLowerCase() ?? '';

        final mobile =
            farmer['mobile']?.toString().toLowerCase() ?? '';

        final village =
            farmer['village']?.toString().toLowerCase() ?? '';
  ...
  Future<void> _deleteFarmer(
    ...
    try {
      await DatabaseService.deleteUser(farmer['id']);
